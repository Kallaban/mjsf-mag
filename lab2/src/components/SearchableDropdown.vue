...
export default {
  ...
  computed: {
    filteredOptions() {
      ...
      return this.options.filter((item) => item.toLowerCase().includes(query));
    },
  },
  methods: {
    navigate(step) {
      ...
      this.highlightedIndex =
        (this.highlightedIndex + step + maxIndex + 1) % (maxIndex + 1);
    },
...
<template>
  <div class="relative">
    <input
      ...
      placeholder="Welche Nummber geffält Ihnen?"
    ...
    <button
      v-if="
        selectedOptions &&
        (multiple ? selectedOptions.length > 0 : selectedOptions)
      "
    ...
    >
      <li
        ...
        :class="[
          ...
          (multiple ? selectedOptions.includes(item) : item === selectedOptions)
